...
pub fn declare_single_blocker_test() {
  ...
  assert state.blocking_creatures == [combat.BlockPair(blocker.id, attacker.id)]
  ...
  let assert Ok(state) =
    action.dispatch(
      ...
      action.DeclareBlockers(2, [
        combat.BlockPair(blocker1.id, attacker1.id),
        combat.BlockPair(blocker2.id, attacker2.id),
      ]),
    )
...
pub fn cannot_block_with_tapped_creature_test() {
  ...
  let result =
    action.dispatch(
      state,
      action.DeclareBlockers(2, [
        combat.BlockPair(blocker.id, attacker.id),
      ]),
    )
...
pub fn cannot_block_creature_attacking_different_player_test() {
  ...
  let state = state.new_multiplayer(4)
  ...
  let assert Ok(state) =
    action.dispatch(
      state,
      action.DeclareAttackers(1, [
        combat.AttackPair(attacker_p2.id, combat.AttackPlayer(2)),
        combat.AttackPair(attacker_p3.id, combat.AttackPlayer(3)),
      ]),
    )
  let state = pass(state)
  ...
  let result =
    action.dispatch(
      state,
      action.DeclareBlockers(2, [
        combat.BlockPair(blocker_p2.id, attacker_p3.id),
      ]),
    )
...
pub fn cannot_block_with_same_creature_twice_test() {
  ...
  let result =
    action.dispatch(
      state,
      action.DeclareBlockers(2, [
        combat.BlockPair(blocker.id, attacker1.id),
        combat.BlockPair(blocker.id, attacker2.id),
      ]),
    )
